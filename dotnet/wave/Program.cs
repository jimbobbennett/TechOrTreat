...
while(true)
{
    ...
    for(int x = 0; x < ledCount; ++x) { 
        // This sets a period of 2 pi so we should get a full transition along the length of the LED strip.
        var value = (float)(Math.Sin(((position + x) / (float)ledCount) * 6.28f) + 1) / 2.0f;
        image.SetPixel(x, 0, Lerp(Color.Blue, Color.Red, value));
    }
